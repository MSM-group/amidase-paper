...
setwd("data/HPLC/252/amidase_substrate_screening")
...
plot <- ggplot(final_cleaned_df_no_outliers %>% filter(sample_name == "S146A") %>%filter(peak %in% c("vorinostat", "diuron", "acetylsulfamethoxazole", "propanil", "atorvastatin")), 
...
df_long <- df_removal %>%
  filter(peak %in% c("vorinostat", "diuron", "acetylsulfamethoxazole", "propanil", "atorvastatin")) %>%
  pivot_longer(
...
average_iqr_plot <- df_stats_median %>%
  ...
  labs(title = "Relative removal median and IQR with blue IQR lines of S146A control (252nm)", x = "Enzyme", y = "Relative removal (%)", color = "Peak")

# Print the plot
print(average_iqr_plot)
ggsave("boxplot_252.png", plot = average_iqr_plot, width = 16, height = 8, dpi = 300)
write.csv(df_stats_median, "data_252nm.csv", row.names = FALSE)
...
hits <- df_stats_combined_filled %>%
  ...
  distinct() %>%
  mutate(wavelength = 252)

# Write the data to a new Excel file
write_xlsx(hits, "hits_252.xlsx")
