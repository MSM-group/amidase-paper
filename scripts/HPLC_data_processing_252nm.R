...
data_dir <- "data/HPLC/252/amidase_substrate_screening"
...
plot <- ggplot(cleaned_df_filter, aes(x = sample_name, y = Area, fill = timepoint, color = batch)) +
  ...
  labs(
    title = "Distribution of Area by Peak and Timepoint of inactivated enzyme (S146A) 252nm",
    x = "Sample Name",
    y = "Area"
  ) +
  theme_minimal() +
...
identify_outliers_iqr <- function(df, column) {
  ...
  upper_bound <- Q1 + 1.5 * IQR_value  # Corrected from Q3 + 1.5 * IQR_value to Q1 + 1.5 * IQR_value
  df <- df %>%
...
# Plot the data with median and IQR
average_iqr_plot <- df_stats_median %>%
  ...
  labs(
    title = "Relative removal median and IQR with blue IQR lines of S146A control (252nm)",
    x = "Enzyme",
    y = "Relative removal (%)",
    color = "Peak"
  )
...
write.csv(df_stats_median, "data/processed/data_252nm.csv", row.names = FALSE)
...
write_xlsx(hits, "data/processed/screening_hits/hits_252.xlsx")
