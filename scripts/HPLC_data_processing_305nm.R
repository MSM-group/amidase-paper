...
data_dir <- "data/HPLC/305/amidase_substrate_screening"
...
data_list <- setNames(lapply(file_list, function(x) {
  read_excel(
    ...
    sheet = "Integration",
    range = "A40:D60",
    skip = 1  # Skip the first row within the range as it contains headers
  )
}), basename(file_list))

# Clean up the data frames in the list
...
# List of specific names for cleanup
specific_names <- c(
  "ACE", "diuron", "Desatinib", "CAP", "atenolol", "Acetylsulfamethoxazole",
  "Propanil", "Atovarstatine", "chlorpropham", "Metoclopramid",
  "Rufinamide", "vorinostat"
)

# Clean and process the combined data frame
cleaned_df <- combined_df %>% 
  filter(!is.na(`Peak Name`)) %>%  # Remove all empty imported rows
  mutate(
    RT = round(as.numeric(`Retention Time`), 1),  # Convert Retention Time to numeric and round
    Area = round(as.numeric(Area), 1)             # Convert Area to numeric and round
  ) %>%
  separate(
    Sample,
    ...
    extra = "merge",
    fill = "right"
  ) %>%  # Split 'Sample' into multiple columns
  mutate(
    batch = ifelse(grepl("\\.xls$", batch), batch, NA),  # Assign batch if ends with .xls
  ...
  filter(!is.na(Area)) %>%                               # Remove entries with no peak area
  mutate(name = gsub("100uM", "", name)) %>%             # Remove '100uM' from name
  rename(sample_name = name) %>%                         # Rename 'name' to 'sample_name'
  select(-`Retention Time`) %>%                          # Remove 'Retention Time' column
  rename(peak = `Peak Name`) %>%                         # Rename 'Peak Name' to 'peak'
  mutate(sample_name = gsub("^AB\\s+", "AB", sample_name)) %>%  # Clean up sample names starting with 'AB '
  filter(!(sample_name %in% specific_names & sample_name != peak)) %>%  # Remove erroneous peaks
  group_by(sample_name, replicate, pool, timepoint, batch, peak) %>%
  filter(Area == max(Area)) %>%                          # Keep the peak with the maximum area
  ungroup() %>%
  mutate(
    replicate = case_when(
      replicate == "A" ~ 1,
...
# Assign batch numbers based on sample_name patterns
...
# Function to fill missing peaks for compounds completely removed
...
# Rename sample names and peaks for consistency
...
plot <- ggplot(cleaned_df_filter, aes(x = sample_name, y = Area, fill = timepoint, color = batch)) +
  ...
  facet_wrap(~peak, scales = "free_y", ncol = 4) +     # Facet by peak with independent y-axes
  labs(
    title = "Distribution of Area by Peak and Timepoint of abiotic control (AB) and inactivated enzyme (S146A) 305nm",
    x = "Sample Name",
    y = "Area"
  ) +
  theme_minimal() +
  theme(
    ...
    strip.text.y = element_text(angle = 0)                # Ensure y-axis facet labels are readable
  )
...
# Identify outliers in the Area column
...
plot <- ggplot(
  final_cleaned_df_no_outliers %>%
    filter(sample_name == "S146A") %>%
    filter(peak %in% c("capecitabine", "metoclopramide", "dasatinib")), 
  aes(x = sample_name, y = Area, fill = timepoint, color = peak)
) +
  geom_boxplot(outlier.shape = NA) +  # Create boxplots; hide outliers
  geom_jitter(width = 0.2, alpha = 0.5, size = 2.5) +  # Add jittered points
  stat_summary(
  ...
  labs(
    title = "Distribution of Area by Peak and Timepoint of inactivated enzyme (S146A) 305nm",
    x = "Sample Name",
    y = "Area"
  ) +
  theme_minimal() +
...
df_long <- df_removal %>%
  filter(peak %in% c("capecitabine", "dasatinib", "metoclopramide")) %>%
  pivot_longer(
...
# Calculate median and IQR for each enzyme
...
# Join summary statistics back to the data
...
average_iqr_plot <- df_stats_median %>%
  ...
  labs(
    title = "Relative removal median and IQR with blue IQR lines of S146A control (305nm)",
    x = "Enzyme",
    y = "Relative removal (%)",
    color = "Peak"
  )
...
write.csv(df_stats_median, "data/processed/data_305nm.csv", row.names = FALSE)

##############
### EXCEL EXPORT ###

# Prepare S146A statistics
...
# Combine statistics with the main data
...
# Function to fill missing values with S146A statistics
...
# Identify hits based on the threshold value
...
# Write the hits data to a new Excel file
write_xlsx(hits, "data/processed/screening_hits/hits_305.xlsx")
