...
setwd("data/HPLC/305/amidase_substrate_screening")
...
plot <- ggplot(final_cleaned_df_no_outliers %>% filter(sample_name == "S146A") %>%filter(peak %in% c("capecitabine","metoclopramide","dasatinib")), 
  ...
  labs(title = "Distribution of Area by Peak and Timepoint of inactivated enzyme (S146A) 305nm",
       x = "Sample Name",
       y = "Area") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),  # Set x-axis labels to be vertical
...
average_iqr_plot <- df_stats_median %>%
  ...
  labs(title = "Relative removal median and IQR with blue IQR lines of S146A control (305nm)", x = "Enzyme", y = "Relative removal (%)", color = "Peak")

# Print the plot
print(average_iqr_plot)
ggsave("boxplot_305.png", plot = average_iqr_plot, width = 16, height = 8, dpi = 300)
write.csv(df_stats_median, "data_305nm.csv", row.names = FALSE)
